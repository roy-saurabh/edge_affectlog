...
const INCLUDED = [
  { icon: Database,  label: "Dataset Audit Engine",    desc: "Full pipeline: schema validation, xAPI normalization, PII scan, metrics.", color: "#67E8F9" },
  { icon: Shield,    label: "Privacy Defaults",        desc: "No raw datasets stored. Pseudonymisation. Metadata-only exports.",         color: "#86EFAC" },
  { icon: Cpu,       label: "Model Adapters",          desc: "Pluggable model interface. Feature importance, model cards.",              color: "#C4B5FD" },
  { icon: Package,   label: "Assessment Recipes",      desc: "YAML-defined pipeline specs. Inokufu, Maskott, generic templates.",        color: "#93C5FD" },
  { icon: Code2,     label: "OpenAPI Backend",         desc: "All workflows exposed via OpenAPI 3.1. Machine-readable contracts.",       color: "#A7F3D0" },
  { icon: Lock,      label: "RBAC + Admin Onboarding", desc: "Admin-approved registrations. Role-based access. Audit log.",             color: "#86EFAC" },
  { icon: Terminal,  label: "Docker Compose",          desc: "PostgreSQL, Redis, local artifact storage. Single command startup.",       color: "#67E8F9" },
  { icon: Users,     label: "Community Support",       desc: "GitHub issues, discussions, contributor guide, and recipes.",             color: "#FCD34D" },
];
      ...
      <div className="max-w-4xl mx-auto px-5 md:px-8">
        ...
        <FadeUp delay={0.1}>
          <div className="rounded-2xl overflow-hidden border" style={{ borderColor: "rgba(203,213,225,0.12)" }}>
            ...
            >
              <div>Feature</div>
              <div className="text-center text-[#67E8F9]">Community</div>
              <div className="text-center text-[#C4B5FD]">Managed Cloud</div>
            </div>
            {rows.map((row, i) => (
              <div
                key={row.feature}
                className="grid grid-cols-3 px-5 py-3.5 text-sm border-t"
                style={{ borderColor: "rgba(203,213,225,0.07)", background: i % 2 === 0 ? "rgba(255,255,255,0.01)" : "transparent" }}
              >
                <div className="text-slate-300">{row.feature}</div>
                <div className="text-center">
                  {row.community === true   ? <CheckCircle2 size={15} className="text-[#67E8F9] mx-auto" />
                   : row.community === false ? <span className="text-slate-600">—</span>
                   : <span className="text-xs text-slate-400">{String(row.community)}</span>}
                </div>
                <div className="text-center">
                  {row.managed === true   ? <CheckCircle2 size={15} className="text-[#C4B5FD] mx-auto" />
                   : row.managed === false ? <span className="text-slate-600">—</span>
                   : <span className="text-xs text-[#C4B5FD]">{String(row.managed)}</span>}
                </div>
              </div>
            ))}
          </div>
          ...
          <FadeUp>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 text-sm text-slate-400 hover:text-[#67E8F9] px-5 py-2.5 transition-colors"
              >
            ...
            {paths.map((p, i) => {
              ...
              return (
                <FadeUp key={p.label} delay={i * 0.06}>
                  ...
                  >
                    <Icon size={16} className="text-slate-500 group-hover:text-[#67E8F9] mb-3 transition-colors" aria-hidden="true" />
